...
def generate_student_pdf_report(user_id: int, filter_mode: str = None, subject: str = "all", report_type: str = "full", timeframe: str = "all") -> str:
    if filter_mode:
        if filter_mode == "last_1_month_data":
            timeframe, report_type = "month", "full"
        elif filter_mode == "last_1_month_quiz":
            timeframe, report_type = "month", "summary"
        elif filter_mode == "all_months_data":
            timeframe, report_type = "all", "full"
        elif filter_mode == "all_months_quiz":
            timeframe, report_type = "all", "summary"
        elif filter_mode == "saved_questions_only":
            timeframe, report_type = "all", "saved"
    ...
    try:
        ...
        if report_type == "saved":
            query = "SELECT * FROM saved_questions WHERE user_id = %s"
            params = [user_id]
            
            if timeframe == "today":
                query += " AND saved_at LIKE %s"
                params.append(f"{today_str}%")
            elif timeframe == "3days":
                query += " AND substring(saved_at from 1 for 10) >= %s"
                params.append(three_days_ago_str)
            elif timeframe == "month":
                query += " AND substring(saved_at from 1 for 10) >= %s"
                params.append(month_ago_str)
        ...
        pdf_filename = f"{username_clean}_{user_id}_{subject}_{report_type}_{timeframe}_report.pdf"
        pdf_path = os.path.join(USER_PROFILES_DIR, pdf_filename)

        html_content = generate_html_report(u, attempts, saved_qs, str(rank), percentile, subject, report_type, timeframe)
