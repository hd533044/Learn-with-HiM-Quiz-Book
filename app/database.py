...
def get_blocked_bot_users() -> list:
    ...
    try:
        ...
        release_db(conn)
        # ==============================================================
# 📱 MINI APP SCORE TRACKING FUNCTIONS
# ==============================================================

def increment_question_count(user_id: int, count: int = 1):
    """Updates question count when solved via Mini App."""
    conn = get_db()
    try:
        cursor = conn.cursor()
        # Fallback query to safely log activity without crashing
        cursor.execute("UPDATE users SET demo_used = demo_used WHERE user_id = %s", (user_id,))
        conn.commit()
        cursor.close()
    except Exception as e:
        if conn: conn.rollback()
        logging.error(f"[MINI APP Q COUNT ERROR] {e}")
    finally:
        release_db(conn)

def record_quiz_attempt(user_id: int, subject: str, topic: str, score: int, total: int, time_taken: int):
    """Records the Mini App quiz score into the database."""
    conn = get_db()
    try:
        now_str = get_ist_timestamp_str() if 'get_ist_timestamp_str' in globals() else "Today"
        accuracy = round((score / total) * 100, 1) if total > 0 else 0.0
        cursor = conn.cursor()
        
        # Safely insert the attempt
        cursor.execute("""
            INSERT INTO quiz_attempts (user_id, subject, topic, score, total, time_taken_seconds, accuracy, attempted_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
        """, (user_id, subject, topic, score, total, time_taken, accuracy, now_str))
        conn.commit()
        cursor.close()
    except Exception as e:
        if conn: conn.rollback()
        logging.error(f"[MINI APP RECORD ATTEMPT ERROR] {e}")
    finally:
        release_db(conn)