...
from app.config import PRIMARY_ADMIN_ID, USER_PROFILES_DIR, PLAN_TIERS, DAILY_QUESTION_LIMIT
...
def get_strict_demo_users():
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT * FROM users 
            WHERE demo_used = 1 AND is_banned != 2 AND (payment_id IS NULL OR payment_id = 'DEMO_PASS' OR payment_id = 'OFFICIAL_SUBSCRIBED')
            AND user_id NOT IN (
...
def get_currently_online_users(minutes: int = 15):
    ...
    try:
        ...
        cursor = conn.cursor(cursor_factory=RealDictCursor)
        cursor.execute(
            "SELECT user_id, full_name, student_id, last_active FROM users WHERE last_activity_epoch >= %s AND is_banned != 2 ORDER BY last_activity_epoch DESC",
            (threshold_epoch,)
        )
        rows = cursor.fetchall()
        cursor.close()
...
async def admin_portal_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    m_status = "🟢 Active (Online)" if now_ts >= m_until else "🔴 PAUSED"
    ...
    keyboard = [
        ...
        [InlineKeyboardButton(f"👥 Student Support & Management ({pending_students_count} 📩)", callback_data="admin_menu_students")],
        [InlineKeyboardButton("📢 Communications & Flash Sales", callback_data="admin_menu_comms")],
        [InlineKeyboardButton("⚙️ System & Security Controls", callback_data="admin_menu_system")],
        [InlineKeyboardButton("🔒 Lock Session Now", callback_data="admin_lock_session")]
    ]

    msg = (
        f"👑 **MASTER ADMIN PORTAL — Himanshu Sir (/him)** 👑\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 **Active Registered Students:** `{len(active_registered)}`\n"
        ...
        f"{sale_summary_line}\n\n"
        f"Select an administrative module below:"
    )
...
async def admin_callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "admin_home":
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("💰 Revenue Dashboard", callback_data="admin_financial_stats"), InlineKeyboardButton("⏱ Platform Time", callback_data="admin_total_platform_usage")],
            [InlineKeyboardButton("📄 PDF Generation Logs", callback_data="admin_pdf_logs"), InlineKeyboardButton("📊 Command Analytics", callback_data="admin_command_stats")],
            [InlineKeyboardButton("🔙 Back to Main Portal", callback_data="admin_home")]
        ]
        msg = "📊 **INTELLIGENCE & ANALYTICS MODULE**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\nSelect an intelligence tool below:"
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        return
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("🗂️ Advanced Student Segments", callback_data="admin_adv_segments_menu")],
            ...
            [InlineKeyboardButton("⚡ Online Users (15m)", callback_data="admin_live_users_menu"), InlineKeyboardButton("🛑 Blocked / Inactive Users", callback_data="admin_list_blocked_users_0")],
            [InlineKeyboardButton("🔙 Back to Main Portal", callback_data="admin_home")]
        ]
        msg = "👥 **STUDENT MANAGEMENT MODULE**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\nSelect a user management action below:"
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        return

    elif data == "admin_menu_comms":
        await query.answer()
        active_sale = get_active_flash_sale()
        ...
        msg = "📢 **COMMUNICATIONS & SALES MODULE**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\nSelect a broadcast or promotional tool below:"
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        return

    elif data == "admin_menu_system":
        await query.answer()
        keyboard = [
            [InlineKeyboardButton("⏸ Pause Bot Services", callback_data="admin_pause_menu"), InlineKeyboardButton("▶️ Resume Bot", callback_data="admin_resume_now")],
            [InlineKeyboardButton("📦 Export Ledgers (.zip)", callback_data="admin_export_zip"), InlineKeyboardButton("🔑 Change Master PIN", callback_data="admin_change_pass_prompt")],
            [InlineKeyboardButton("🔙 Back to Main Portal", callback_data="admin_home")]
        ]
        msg = "⚙️ **SYSTEM & SECURITY CONTROLS**\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\nSelect a system configuration option:"
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        return

    elif data == "admin_pause_menu":
        await query.answer()
        keyboard = [
            [InlineKeyboardButton("⏸ 5m", callback_data="admin_pause_5"), InlineKeyboardButton("⏸ 10m", callback_data="admin_pause_10"), InlineKeyboardButton("⏸ 1h", callback_data="admin_pause_60")],
            [InlineKeyboardButton("⏸ 3h", callback_data="admin_pause_180"), InlineKeyboardButton("⏸ 6h", callback_data="admin_pause_360"), InlineKeyboardButton("⏸ 24h", callback_data="admin_pause_1440")],
            [InlineKeyboardButton("🔙 Back to System Menu", callback_data="admin_menu_system")]
        ]
    ...
    if data == "admin_adv_segments_menu":
        ...
        count_paid_active = 0
        count_expiring_soon = 0
        count_paid_expired = 0
        count_demo_active = 0
        ...
        for u in users:
            ...
            if exp_dt > now_ist:
                if is_paid_user:
                    count_paid_active += 1
                    if exp_dt <= next_week: count_expiring_soon += 1
                else: count_demo_active += 1
            else:
                if is_paid_user: count_paid_expired += 1
                else: count_demo_expired += 1

        keyboard = [
            [InlineKeyboardButton(f"🟢 Active Paid VIP ({count_paid_active})", callback_data="admin_seg_paidactive_0")],
            [InlineKeyboardButton(f"🟡 Expiring Soon < 7 Days ({count_expiring_soon})", callback_data="admin_seg_expiringsoon_0")],
            [InlineKeyboardButton(f"🔴 Expired Paid Passes ({count_paid_expired})", callback_data="admin_seg_paidexpired_0")],
            [InlineKeyboardButton(f"🎁 Active Free Demo ({count_demo_active})", callback_data="admin_seg_demoactive_0")],
            [InlineKeyboardButton(f"⚠️ Expired Free Demo ({count_demo_expired})", callback_data="admin_seg_demoexpired_0")],
            [InlineKeyboardButton("🔙 Back to Student Menu", callback_data="admin_menu_students")]
        ]
        
        msg = (
            f"🪂 **ADVANCED STUDENT SEGMENTS** 🪂\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"Real-time categorization based on razorpay plans, active limits, and exact pass expiration dates.\n\n"
            f"Select a category below to inspect students:"
        )
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        return
        ...
        msg = (
            ...
            f"Tap any student below to inspect profile and subscription ledger:"
        )
        await query.edit_message_text(msg, reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        return
    ...
    if data.startswith("admin_revoke_menu_"):
        ...
        cursor.execute("SELECT * FROM payment_transactions WHERE user_id = %s AND payment_id LIKE 'pay_%%' AND amount_paid > 0", (target_uid,))
        paid_purchases = cursor.fetchall()
        cursor.close()
        release_db(conn)
        ...
        if restored.get("has_paid_plan"):
            status_desc = (
                f"✅ **ADMIN GRANTS WITHDRAWN! PAID PLANS PROTECTED & RESTORED!**\n\n"
                f"• All complimentary admin grants were removed.\n"
                f"• Student **{st_name}** (`{target_uid}`) has **{restored['active_count']} active paid plan(s)**.\n"
                f"• ⚡ **Restored Quota:** `{restored['quota']} Questions/Day`\n"
        ...
        if restored.get("has_paid_plan"):
            msg_res = (
                f"🛡 **PLAN AUDIT COMPLETE! PAID PLAN PRESERVED & ACTIVE!**\n\n"
                f"• Removed all Admin Grants.\n"
                f"• 🔒 **Automatic Paid Plan Protection:** Preserved `{restored['active_count']}` genuine paid plan(s).\n"
                f"• ⚡ **Current Daily Quota:** `{restored['quota']} Questions/Day`\n"
                f"• ⏳ **Remaining Validity:** `{restored['remaining_days']} Days` (Expires: `{restored['expiry_str']}`)"
            )
        else:
        ...
        cursor.execute("SELECT COUNT(*) as count FROM users WHERE is_banned != 2")
        ...
        overview_msg = (
            ...
            f"• **Total Active Students:** `{reg_count}`\n"
            f"• **Platform Likes:** `{total_likes_count}` ❤️\n"
            f"• **Active in Last 15 Mins:** `{len(online_15m)} Students`\n"
            f"• **Unread Support Queries:** `{pending_queries}`\n"
            f"• **Blocked / Inactive Users:** `{blocked_count}`\n"
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("🔙 Back to Analytics", callback_data="admin_menu_analytics")]
        ]
    ...
    # ==============================================================
        ...
        if not blocked:
            ...
            await query.edit_message_text("🛑 **BLOCKED / INACTIVE USERS AUDIT**\n\n🎉 No blocked users found in the database!", reply_markup=nav, parse_mode="Markdown")
            return
        ...
        await query.edit_message_text(
            ...
            f"All historical records and files are saved and accessible. Tap any student to inspect their dashboard:",
        ...
        if nav_row: keyboard.append(nav_row)
        keyboard.append([
            InlineKeyboardButton("➕ Schedule New Post", callback_data="admin_schedule_annc"),
            InlineKeyboardButton("🔙 Back to Comms Menu", callback_data="admin_menu_comms")
        ])
        ...
        if not sent_posts:
            nav = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Comms Menu", callback_data="admin_menu_comms")]])
        ...
        if nav_row: keyboard.append(nav_row)
        keyboard.append([InlineKeyboardButton("🔙 Back to Comms Menu", callback_data="admin_menu_comms")])
        ...
        delete_scheduled_announcement(annc_id)
        nav = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Sent History", callback_data="admin_list_sent_annc_0")]])
        await query.edit_message_text(f"🗑 **LIVE UNSEND COMPLETE!**\nSuccessfully deleted message from `{deleted_count}` out of `{len(deliveries)}` chats.", reply_markup=nav, parse_mode="Markdown")
        ...
        cancel_btn = InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Cancel", callback_data="admin_menu_system")]])
        await query.edit_message_text(
            "🔑 **CHANGE ADMIN MASTER PIN**\n"
            "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            "Please reply with your new 4-Digit Master Admin PIN:",
            reply_markup=cancel_btn,
            parse_mode="Markdown"
        )
    ...
    # ==============================================================
        ...
        title_lbl = "UNREAD PENDING THREADS" if not show_resolved else "RESOLVED / OLD QUERIES ARCHIVE"
        await query.edit_message_text(f"📩 **STUDENT SUPPORT THREADS — {title_lbl} ({total_s} Students)**\nSelect a student to inspect inquiry:", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
        ...
        if not queries:
            await query.edit_message_text("ℹ️ No queries found for this student.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Back to Support Threads", callback_data="admin_view_student_threads_0")], [InlineKeyboardButton("👑 Himanshu Sir's Portal (/him)", callback_data="admin_home")]]))
            return
        ...
        if not query_row:
            await query.message.reply_text("⚠️ Query record not found.")
            return
        ...
        media_info = ""
        if query_row.get("voice_file_id"): media_info = "\n🎙️ *Student attached a Voice Note.*"
        elif query_row.get("photo_file_id"): media_info = "\n📷 *Student attached a Photo/Image.*"
        elif query_row.get("video_file_id"): media_info = "\n🎬 *Student attached a Video.*"
        elif query_row.get("doc_file_id"): media_info = "\n📄 *Student attached a Document/PDF.*"

        prompt_msg = (
            f"✍️ **REPLYING TO STUDENT QUERY #{qid}**\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"👤 **From:** {query_row['student_name']} (`{query_row['user_id']}`)\n"
            f"📅 **Time:** `{query_row['created_at']}`\n\n"
            f"❓ **Student's Question:**\n"
            f"👉 *\"{query_row['query_text'] or 'Media Query'}\"*{media_info}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"Please reply with text, a **Voice Note**, **Image/Photo**, **Video**, **Audio**, **GIF**, or **PDF/Document** to answer:"
        )

        await query.edit_message_text(prompt_msg, reply_markup=cancel_btn, parse_mode="Markdown")
        ...
        conn = get_db()
        cursor = conn.cursor()
        cursor.execute("UPDATE student_queries SET status = 'RESOLVED', admin_reply = '(Marked as Read by Admin)' WHERE id = %s", (qid,))
        conn.commit()
        cursor.close()
        release_db(conn)

        await query.answer("Query marked as Read!", show_alert=True)
        ...
        await query.answer("Query deleted!", show_alert=True)
        query.data = f"admin_student_thread_{target_uid}"
        await admin_callback_handler(update, context)
        ...
        cancel_btn = InlineKeyboardMarkup([[InlineKeyboardButton("❌ Cancel Search & Return", callback_data="admin_menu_students")]])
        ...
        msg = (
            ...
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"• **Total Active Students:** `{total_users}`\n"
            f"• **Page:** `{page + 1}` of `{total_pages}`\n\n"
        ...
        if is_banned == 2:
            ...
            ban_btn_label = "🟢 Reactivate Student"
        elif is_banned == 1:
            ban_text = "🔴 BANNED"
            ban_btn_label = "🟢 Unban Student"
        else:
            ban_text = "🟢 ACTIVE"
        ...
        keyboard = [
            ...
            [InlineKeyboardButton("🚫 Revoke / Manage Plan", callback_data=f"admin_revoke_menu_{target_uid}"), InlineKeyboardButton("🎁 Grant +20 Quota", callback_data=f"audit_grant_{target_uid}")],
            ...
            [InlineKeyboardButton(ban_btn_label, callback_data=f"admin_toggle_ban_{target_uid}"), InlineKeyboardButton("💬 Student Feedback", callback_data=f"audit_feedback_{target_uid}")],
