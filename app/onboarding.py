...
async def check_onboarding_maintenance(update: Update) -> bool:
    ...
    if int(time.time()) < m_until:
        msg = "🛠 SERVICE PAUSED\n\nServices are currently paused by Admin. Please try again shortly."
...
async def start_onboarding(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if profile and profile.get("is_verified") and not context.user_data.get("is_editing_profile"):
        ...
        await update.effective_message.reply_text(
            f"⚡ WELCOME BACK, {profile['full_name'].upper()}!\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🚀 LAUNCH QUIZ", callback_data="cmd_quiz"), InlineKeyboardButton("👤 MY PROFILE", callback_data="cmd_profile")],
    ...
    await update.effective_message.reply_text(
        ...
        f"Please reply with your Full Name (at least 4 letters):"
    )
    return NAME
...
async def name_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(clean_letters) < 4:
        await update.message.reply_text(
            "⚠️ INVALID NAME\n\n"
            "Name must contain at least 4 letters. Please enter your Full Name again:"
        )
        return NAME
    ...
    await update.message.reply_text(
        f"✨ WELCOME, {context.user_data['full_name'].upper()}!\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🎯 SELECT TARGET EXAM (STEP 2/7):\n\n"
        f"Choose your examination from below:",
...
async def exam_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        f"🎯 Target Exam: {selected_exam}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📍 SELECT STATE / UT (STEP 3/7):\n\n"
        f"Choose your Indian State or Union Territory:",
...
async def custom_exam_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("awaiting_other_exam"):
        ...
        await update.message.reply_text(
            f"🎯 Target Exam: {context.user_data['target_exam']}\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
            f"📍 SELECT STATE / UT (STEP 3/7):\n\n"
            f"Choose your Indian State or Union Territory:",
            reply_markup=build_state_keyboard()
...
async def state_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        ...
        text=f"📍 Location: {selected_state}, India\n"
             f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
             f"📱 MOBILE VERIFICATION (STEP 4/7):\n\n"
             f"Tap the button below to share your mobile number:",
        reply_markup=markup
    )
    return PHONE
...
async def phone_contact_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.contact:
        ...
        await update.message.reply_text(
            ...
            "Please tap the button below to share your verified mobile number:",
            reply_markup=markup
        )
        return PHONE
...
async def gender_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"Select your Year of Birth below:",
...
async def dob_year_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        f"📅 Birth Year: {selected_year}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🗓 SELECT BIRTH MONTH:\n\n"
        f"Select your Month of Birth below:",
...
async def dob_month_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        f"📅 Birth Period: {selected_month}/{selected_year}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🗓 SELECT BIRTH DAY:\n\n"
        f"Select your Day of Birth below:",
...
async def dob_day_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        ...
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"🔑 ACCOUNT SECURITY (STEP 7/7):\n\n"
        f"Set a 4-Digit Security PIN for your account (e.g. 4321):"
    )
    return PIN_SETUP
...
async def pin_setup_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not pin_input.isdigit() or len(pin_input) != 4:
        await update.message.reply_text("⚠️ PIN must be exactly 4 digits (e.g. 4321). Try again:")
        return PIN_SETUP
    ...
    await update.message.reply_text(
        f"🛡 SECURITY QUESTION\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"Select a question for PIN recovery:",
...
async def sec_ans_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"Your profile is verified and active! Use /myprofile anytime to view your card.",
...
async def edit_profile_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not can_edit:
        msg = f"⏳ PROFILE EDIT LOCKED\n\nProfile edits are allowed once every 30 days.\nTry again in {days_left} days."
        if update.callback_query:
            await update.callback_query.answer(msg, show_alert=True)
        else:
            await update.message.reply_text(msg)
    ...
    warn_msg = (
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n"
        "Profile details can only be edited ONCE EVERY 30 DAYS.\n\n"
        "Do you wish to proceed?"
    )
    warn_markup = InlineKeyboardMarkup([
        [InlineKeyboardButton("✅ YES, PROCEED", callback_data="edit_confirm_yes")],
        [InlineKeyboardButton("❌ CANCEL", callback_data="edit_confirm_no")]
    ])

    if update.callback_query:
        await update.callback_query.message.reply_text(warn_msg, reply_markup=warn_markup)
    else:
        await update.message.reply_text(warn_msg, reply_markup=warn_markup)
...
async def edit_warn_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "edit_confirm_no":
        await query.edit_message_text("❌ Profile edit cancelled.")
        return ConversationHandler.END

    context.user_data["is_editing_profile"] = True
    await query.edit_message_text(
        "✏️ EDIT PROFILE (STEP 1/7)\n\n"
...
async def recovery_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not u:
        await query.edit_message_text("⚠️ Session expired. Please type /start again.")
        return ConversationHandler.END
    ...
    await query.edit_message_text(
        ...
        f"Select a recovery method to reset your 4-digit PIN:",
...
async def recovery_option_router(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "rec_opt_secq":
        ...
        await context.bot.send_message(
            chat_id=query.message.chat_id,
            text=f"📱 PHONE RECOVERY\n\nTap the button below to share your registered phone number:",
            reply_markup=markup
        )
        return REC_PHONE
        ...
        await query.edit_message_text(
            f"👤 NAME & DOB RECOVERY\n"
            f"━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
            f"Reply with your Registered Full Name or DOB (DD-MM-YYYY):"
        )
        return REC_NAME_DOB
...
async def rec_sec_ans_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ans_input != correct_ans:
        ...
        await update.message.reply_text("❌ Incorrect answer. Try again or pick another method:", reply_markup=rec_btn)
        return REC_SEC_ANS

    await update.message.reply_text(
        f"✅ IDENTITY VERIFIED!\n\n"
        f"Student: {u['full_name']} ({u['student_id']})\n\n"
        f"Please enter your New 4-Digit Security PIN:"
    )
    return RESET_PIN
...
async def rec_phone_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not update.message.contact:
        contact_btn = KeyboardButton(text="📱 SHARE VERIFIED MOBILE NUMBER", request_contact=True)
        markup = ReplyKeyboardMarkup([[contact_btn]], one_time_keyboard=True, resize_keyboard=True)
        await update.message.reply_text("⚠️ Tap the button below to share your mobile number:", reply_markup=markup)
        return REC_PHONE
    ...
    if shared_phone != user_phone:
        rec_btn = InlineKeyboardMarkup([[InlineKeyboardButton("🔑 RESET PIN", callback_data="login_forgot_pin")]])
        await update.message.reply_text(
            f"❌ Phone number mismatch for Student ID {u['student_id']}.",
...
async def rec_name_dob_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    rec_btn = InlineKeyboardMarkup([[InlineKeyboardButton("🔑 RESET PIN", callback_data="login_forgot_pin")]])
    await update.message.reply_text(
        "❌ Verification failed. Input does not match our records.",
        reply_markup=rec_btn
    )
    return REC_NAME_DOB
...
async def rec_dob_day_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=query.message.chat_id,
        text=f"✅ DOB VERIFIED!\n\nStudent: {u['full_name']} ({u['student_id']})\n\nPlease enter your New 4-Digit Security PIN:"
    )
    return RESET_PIN
...
async def reset_pin_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not new_pin.isdigit() or len(new_pin) != 4:
        await update.message.reply_text("⚠️ PIN must be exactly 4 digits (e.g. 1234). Try again:")
        return RESET_PIN
    ...
    await update.message.reply_text(
        ...
        f"Your account is fully ready. Tap Launch Quiz below or use /quiz to continue!",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("🚀 LAUNCH QUIZ", callback_data="cmd_quiz"), InlineKeyboardButton("👤 MY PROFILE", callback_data="cmd_profile")]
        ])
    )
