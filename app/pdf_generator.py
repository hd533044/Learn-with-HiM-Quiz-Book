...
def get_devanagari_font():
    ...
    Prevents empty boxes or black squares on production servers.
    """
...
ACTIVE_HINDI_FONT = get_devanagari_font()
...
def clean_str(text) -> str:
    ...
    so matras, halants, and vowel signs bind correctly.
    """
    if text is None:
        return "N/A"
...
def generate_student_pdf_report(user_id: int, filter_mode: str = "last_1_month_data") -> str:
    ...
    try:
        ...
        if filter_mode == "saved_questions_only":
            ...
            img_left = RLImage(logo_left_path, width=0.8*inch, height=0.8*inch) if os.path.exists(logo_left_path) else Paragraph("<b>Logo</b>", body_style_bold)
            img_right = RLImage(logo_right_path, width=0.8*inch, height=0.8*inch) if os.path.exists(logo_right_path) else Paragraph("<b>@LearnwithHiM</b>", body_style_bold)
            ...
            for sq in saved_rows:
                ...
                q_text_clean = clean_str(sq.get('question_text', 'N/A'))
                q_desc = f"{q_text_clean}"
                ...
                saved_table_data.append([
                    ...
                    Paragraph(q_desc, content_cell_style),
                    Paragraph(clean_str(ans_txt), content_cell_style)
                ])
            ...
            def build_styled_question_table(q_list, bg_header, border_color, empty_msg):
                ...
                for q in q_list:
                    ...
                    table_data.append([
                        ...
                        Paragraph(q_txt, content_cell_style),
                        Paragraph(c_ans, content_cell_style)
                    ])
