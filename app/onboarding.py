...
async def edit_warn_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await query.edit_message_text(
        "✏️ **ᴇᴅɪᴛ ᴘʀᴏꜰɪʟᴇ ꜱᴇꜱꜱɪᴏɴ ꜱᴛᴀʀᴛᴇᴅ (ꜱᴛᴇᴘ 1/8)**\n\n"
...
async def name_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🎯 **ᴛᴀʀɢᴇᴛ ᴇxᴀᴍ ꜱᴇʟᴇᴄᴛɪᴏɴ (ꜱᴛᴇᴘ 2/8):**\n"
...
async def exam_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    country_buttons = InlineKeyboardMarkup([
        [InlineKeyboardButton("🇮🇳 ɪɴᴅɪᴀ", callback_data="country_India"), InlineKeyboardButton("🌎 ᴏᴛʜᴇʀ ᴄᴏᴜɴᴛʀʏ", callback_data="country_OTHER")]
    ])
    await query.edit_message_text(
        f"🎯 ꜱᴇʟᴇᴄᴛᴇᴅ ᴛᴀʀɢᴇᴛ: `{selected_exam}`\n\n"
        f"🌍 **ᴄᴏᴜɴᴛʀʏ ꜱᴇʟᴇᴄᴛɪᴏɴ (ꜱᴛᴇᴘ 3/8):**\n"
        f"ᴘʟᴇᴀꜱᴇ ᴄʜᴏᴏꜱᴇ ʏᴏᴜʀ ᴄᴏᴜɴᴛʀʏ ꜰʀᴏᴍ ʙᴇʟᴏᴡ:",
...
async def country_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice == "OTHER":
        ...
        await query.edit_message_text("✍️ ᴘʟᴇᴀꜱᴇ ᴛʏᴘᴇ ᴛʜᴇ ɴᴀᴍᴇ ᴏꜰ ʏᴏᴜʀ ᴄᴏᴜɴᴛʀʏ:")
        return COUNTRY

    context.user_data["country"] = "India"
    await query.edit_message_text(
        "📍 **ɪɴᴅɪᴀɴ ꜱᴛᴀᴛᴇ / ᴜᴛ ꜱᴇʟᴇᴄᴛɪᴏɴ (ꜱᴛᴇᴘ 4/8):**\n\n"
        "ᴘʟᴇᴀꜱᴇ ꜱᴇʟᴇᴄᴛ ʏᴏᴜʀ ꜱᴛᴀᴛᴇ ᴏʀ ᴜɴɪᴏɴ ᴛᴇʀʀɪᴛᴏʀʏ ꜰʀᴏᴍ ᴛʜᴇ ɪɴᴛᴇʀᴀᴄᴛɪᴠᴇ ʟɪꜱᴛ ʙᴇʟᴏᴡ:",
...
async def custom_country_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if context.user_data.get("awaiting_other_country"):
        ...
        await update.message.reply_text(
            f"🌍 ᴄᴏᴜɴᴛʀʏ: `{context.user_data['country']}`\n\n"
            f"📱 **ᴍᴏʙɪʟᴇ ᴠᴇʀɪꜰɪᴄᴀᴛɪᴏɴ (ꜱᴛᴇᴘ 5/8):**\n"
...
async def sec_ans_step(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    save_user_profile(
        ...
        sec_a=ans_input,
        country=context.user_data.get("country", "ɪɴᴅɪᴀ"),
